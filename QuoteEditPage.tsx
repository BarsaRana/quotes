...
// ========== TOASTER COMPONENT ==========
function Toaster({ toast, onClose }: { toast: null | { type: "success" | "error", message: string }, onClose: () => void }) {
  React.useEffect(() => {
    if (toast) {
  ...
  return (
    <div
      style={{
        ...
        minWidth: 230,
        minHeight: 44,
        background: toast.type === "success" ? "#16a34a" : "#be123c",
        color: "#fff",
        padding: "14px 26px",
        borderRadius: 12,
        boxShadow: "0 8px 32px rgba(14,30,59,.17)",
        fontWeight: 700,
        fontSize: 16,
        display: "flex",
        alignItems: "center",
...
function MiniComboBox({
  ...
  disabled?: boolean;
  allowCreate?: boolean;
  onCreate?: (label: string) => Promise<{ id: Id, label: string }> | { id: Id, label: string };
  onChange: (id: Id, option?: { id: Id, label: string }) => void;
  style?: CSSProperties;
}) {
  ...
  const pick = (opt: { id: Id; label: string }) => { onChange(opt.id, opt); setQ(opt.label); setOpen(false); };
...
export default function QuoteEditPage() {
  ...
  // ========== Load lookups and quote ==========
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const [clientsData, regionsData] = await Promise.all([
          getClients(),
          getRegions()
        ]);
        ...
        // Fetch quote details
        const { data: quote } = await axios.get(`${API_BASE}/quotes/${id}`);

        // Map client/region name to ID for dropdowns (if backend doesn't send client_id/region_id)
        const clientMatch = clientsData.find(c => c.name === quote.client);
        setSelectedClientId(clientMatch ? clientMatch.id.toString() : "");
        const regionMatch = regionsData.find(r => r.name === quote.region);
        setRegionId(regionMatch ? regionMatch.id.toString() : "");

        setRiskPct((quote.risk_percent ?? "").toString());
        setQuoteStatus(quote.status === "Final" ? "Final" : "draft");
        setSummaryProducts(quote.breakdown?.products ?? []);

        // Set product selections from breakdown.products if available
        if (quote.breakdown?.products?.length) {
          setProductSelections(
            quote.breakdown.products.map((p: any) => p.product_id?.toString())
          );
          // Flatten all tasks for editing
          const allTasks: TaskBreakdown[] = [];
          for (const p of quote.breakdown.products) {
            for (const t of p.tasks) {
              allTasks.push({
                ...t,
                id: t.ref_id,
                qty: t.qty ?? 1,
                equipment: (t.equipment ?? []).map((eq: any) => ({
                  ...eq,
                  name: eq.name ?? eq.description ?? "",
                })),
                materials: (t.materials ?? []).map((mat: any) => ({
                  ...mat,
                  name: mat.name ?? mat.description ?? "",
                })),
                labour: (t.labour ?? []).map((lab: any) => ({
                  ...lab,
                  name: lab.name ?? lab.description ?? "",
                })),
              });
            }
          }
        ...
        setSupportItems(
          (quote.breakdown?.support ?? []).map((it: any) => ({
            ...it,
            name: it.name ?? it.description ?? "",
          }))
        );
      } catch (e) {
        setError("Failed to load quote or lookups");
      } finally {
        setLoading(false);
      }
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      catch (e) { setError("Failed to load products for region."); }
  ...
  const [flatEquipment, setFlatEquipment] = useState<Item[]>([]);
  const [flatLabour, setFlatLabour] = useState<Item[]>([]);
  useEffect(() => {
    // For cost calculations only
    let mats: Item[] = [], eqs: Item[] = [], lab: Item[] = [];
    for (const t of breakdown.tasks || []) {
      (t.materials || []).forEach(mat => mats.push(mat));
      (t.equipment || []).forEach(eq => eqs.push(eq));
      (t.labour || []).forEach(l => lab.push(l));
    }
    setFlatMaterials(mats);
    setFlatEquipment(eqs);
    setFlatLabour(lab);
  }, [breakdown.tasks]);
  ...
  const baseCost = useMemo(() => sumItems(flatMaterials) + sumItems(flatEquipment) + sumItems(flatLabour), [flatMaterials, flatEquipment, flatLabour]);
  ...
  // ========== Save Handler ==========
  async function handleSave(status: "Final" | "draft") {
    setError("");
    const selectedProducts = productSelections.filter(Boolean);
    if (!selectedClientId) {
      setError("Please pick or create a client.");
      setToast({ type: "error", message: "Please pick or create a client." });
      return;
    }
    if (!regionId || !selectedProducts.length) {
      setError("Please select Region and at least one Product/SOR first.");
      setToast({ type: "error", message: "Please select Region and at least one Product/SOR first." });
      return;
    }
    try {
      setLoading(true);
      const nestedBreakdown = buildNestedBreakdown(selectedProducts, products, breakdown.tasks);
      const payload = {
      ...
      setToast({ type: "success", message: status === "Final" ? "Quote updated successfully!" : "Draft saved!" });
      setTimeout(() => navigate(`/quotes/${id}`), 1200);
    } catch (e) {
      setError("Update failed. Check console & API.");
      setToast({ type: "error", message: "Failed to save quote. Please check your input and try again." });
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="layout" style={styles.content}>
        <section>
          <Card title="Edit Quote">
            ...
            <div className="actions actions-center custom-actions">
              ...
              >
                {loading && quoteStatus === "Final" ? "Saving…" : "Update"}
              </button>
              <button
                ...
                className="btn secondary"
              >
                {loading && quoteStatus === "draft" ? "Saving…" : "Save as Draft"}
              </button>
              <button
        ...
        <section>
          ...
          >
            ...
            <div className="kv">
              <SummaryRow label="Client" value={displayName(clients, selectedClientId)} />
            ...
            <hr className="hr" />
            {/* Grouped quote summary matching QuotesDetail */}
            {summaryProducts.length > 0 && summaryProducts.map((prod, pi) => (
              <div key={prod.product_id || pi} style={{ marginBottom: 28 }}>
                <div style={{ fontWeight: 700, fontSize: 16, margin: "18px 0 6px" }}>
